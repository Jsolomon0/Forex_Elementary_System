...
class PerformanceAnalyzer:
    ...
    def _loss_breakdown(self, max_rows=5):
        if self.df.empty or 'pnl' not in self.df.columns:
            return {}
        losses = self.df[self.df['pnl'] < 0]
        if losses.empty:
            return {}

        breakdown = {}
        groups = [
            ("strategy", "Losses By Strategy"),
            ("structure", "Losses By Regime"),
            ("type", "Losses By Direction"),
            ("entry_hour", "Losses By Entry Hour"),
        ]
        ...
        for col, label in groups:
            if col not in losses.columns:
                continue
            grouped = (
                losses.groupby(col)['pnl']
                .agg(['count', 'sum', 'mean'])
                .sort_values('sum')
                .head(max_rows)
                .reset_index()
            )
            breakdown[label] = grouped.to_dict(orient='records')

        return breakdown

    def _format_breakdown(self, records, label_key):
        if not records:
            return "N/A"
        parts = []
        for row in records:
            ...
            parts.append(
                f"{label} (n={row['count']}, sum={row['sum']:.2f}, mean={row['mean']:.2f})"
            )
        return "; ".join(parts)
    ...
    def generate_report(self):
        ...
        if loss_breakdown:
            print_report_dict["--- LOSS BREAKDOWN (TOP 5 BY NET LOSS) ---"] = ""
            if "Losses By Strategy" in loss_breakdown:
                print_report_dict["Losses By Strategy"] = self._format_breakdown(
                    loss_breakdown["Losses By Strategy"], "strategy"
                )
            if "Losses By Regime" in loss_breakdown:
                print_report_dict["Losses By Regime"] = self._format_breakdown(
                    loss_breakdown["Losses By Regime"], "structure"
                )
            if "Losses By Direction" in loss_breakdown:
                print_report_dict["Losses By Direction"] = self._format_breakdown(
                    loss_breakdown["Losses By Direction"], "type"
                )
            if "Losses By Entry Hour" in loss_breakdown:
                print_report_dict["Losses By Entry Hour"] = self._format_breakdown(
                    loss_breakdown["Losses By Entry Hour"], "entry_hour"
                )
